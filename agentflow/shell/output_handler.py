...
def _read_fill_tokens(project_root: pathlib.Path) -> int | None:
    ...
    try:
        ...
        # Silent fallback for stale/missing context_fill
        pass
    return None
...
def record_task_tokens(manager, task_id: str, delta: int) -> None:
    ...
    try:
        ...
        manager._log_audit({"event": "record_task_tokens_read_error", "error": str(e)})
    ...
    if complete_m:
        ...
        if lp.parent.exists():
            try:
                ...
                manager._log_audit({"event": "verbosity_log_write_error", "error": str(e)})
