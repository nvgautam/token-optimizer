...
def update_last_current_round_mtime(session_manager) -> None:
    ...
    try:
        ...
        session_manager._log_audit({"event": "mtime_stat_error", "error": str(e)})
    ...
    try:
        ...
        if ledger_path.exists():
            try:
                ...
                session_manager._log_audit({"event": "ledger_read_error", "error": str(e)})
        calibrate_capacity(session_manager._project_root, current_start_pct)
    except Exception as e:
        session_manager._log_audit({"event": "calibrate_capacity_error", "error": str(e)})
...
def handle_session_exit(session_manager, exit_code: int) -> None:
    ...
    try:
        ...
        session_manager._log_audit({"event": "session_exit_transition_error", "error": str(e)})
