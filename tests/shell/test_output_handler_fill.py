...
class TestFillTokenGuard(unittest.TestCase):
    ...
    def test_handle_output_uses_fill_tokens_when_fresh(self):
        """T-209: fill-token-based output trigger removed — no trigger_handoff on task complete."""
    ...
    def test_handle_output_falls_back_to_terminal_count_when_stale(self):
        """T-209: accumulated-token output trigger removed — no trigger_handoff even above primary."""
        with tempfile.TemporaryDirectory() as tmpdir:
            manager = self._create_mock_manager()
            project_root = pathlib.Path(tmpdir)
            manager._project_root = project_root
    ...
    def test_handle_output_token_evaluation_gated(self):
        """T-209: token_evaluation audit removed with accumulated-token trigger block."""
        with tempfile.TemporaryDirectory() as tmpdir:
            manager = self._create_mock_manager()
            project_root = pathlib.Path(tmpdir)
            manager._project_root = project_root
            ...
            # T-209: token_evaluation audit removed with the accumulator trigger block
            assert len(token_eval_calls) == 0
