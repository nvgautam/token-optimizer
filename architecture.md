...
**Model (calibrated, T-085):** tok/turn is no longer an invented constant. `agentflow/reporting/handoff_savings.py` computes `(final_ctx - initial_ctx) / n_turns` per closed session in `agentflow_ledger.json` (N=90 as of 2026-07-02) and takes the **p25 percentile** — conservative, undershoot rather than overshoot, per savings-claim credibility policy — as the calibrated constant: **~1,127 tok/turn**, replacing the earlier invented 500. Bucketing by session type (oracle vs orchestrator) was attempted by joining ledger `end_time` to `.agentflow/telemetry.jsonl` handoff events on nearest timestamp, but the join proved unreliable on real data — telemetry logs only 57 handoff events against 90 closed sessions, and matched timestamps are frequently minutes apart (median delta ~400s) — so it falls back to one global, unbucketed p25 pool — the methodology label still surfaces the population-level tag skew as an honest coarse signal ("predominantly orchestrator sessions, 43 of 60 tagged handoff events") rather than a bare "no join" note. Fed into the same triangular-sum model against a representative session (median measured length = 68 turns, handoff inserted where accumulated context crosses the 40K threshold at the calibrated rate), the projected reduction is **~45% (1.19M of 2.64M tokens)** for that session shape. This is a **modeled projection**, not a measured savings figure — surfaced in `agentflow report` as a separate "Modeled Projections" line item with an explicit methodology label, and excluded from the report's measured `total_saved` figure.
