...
| Headroom stale-Read protection | RESOLVED | Incident 2026-07-01: /orchestrate flagged garbled-looking Read/Grep results on design_status.md + rate_calibration_claude.json, correctly paused instead of acting. Traced in headroom's installed source (not guesswork): `DEFAULT_EXCLUDE_TOOLS` (Read/Glob/Grep/Write/Edit) already protects those tools from ContentRouter; cli.py's `headroom wrap claude` (no flags) leaves that default intact. Real risk is `ReadLifecycleConfig` (`compress_stale=True`), which runs independent of tool exclusion and rewrites a prior-turn Read of a file into a `[Read content stale ... re-read the file]` marker once that file is later edited — expected given AgentFlow's living-doc architecture (design_status.md/tasks.json/rate_calibration*.json are rewritten mid-session by AgentFlow's own tooling), not corruption. Fix: skill-level rule (always fresh same-turn Read before any gate decision; treat a stale/superseded marker as a re-read signal, never as data) + a static test asserting the headroom invocation never silently gains `--intercept-tool-results`/`--mode` flags that would defeat the exclusion default — T-079. |
