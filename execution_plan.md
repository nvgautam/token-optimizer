...
| C | T-162 ‖ T-210 ‖ T-233 ‖ T-234 ‖ T-235 ‖ T-236 ‖ T-237 (parallel) | oracle.md size split + test cache leak + verbosity rule + context bundle temp file + enforce worker-always rule + remove OWNS gate + post-merge conflict resolution + restart new-SID + re-derive |
| D | T-178 ‖ T-211 ‖ T-231 ‖ T-238 (parallel) | Hook audit log spike + Gemini lifecycle spike + SQLite spike + /debug skill spec |
