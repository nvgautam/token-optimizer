...
## Startup
Execute `commands/claude/orchestrator/startup.md` steps: Step 1 (Persona), Step 2 (Rate check), Step 2b (Index execution_plan.md only), Step 3 (Oracle gate on design_status.md UNRESOLVED items; stop if count > 0), Step 3b (Cache), Step 4 (Load state: read round table, identify pending tasks), Step 4b (Select round: announce "Picking up Round X: T-xxx"), and Step 5 (Load prior calibration from rate_calibration_claude.json).
