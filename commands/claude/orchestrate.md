...
**Rate-pacing & EWMA (disabled):** Track observed_costs via TOKENS: reports. Per-task cost: sample_count < 7 → 2500 (static default); sample_count ≥ 7 and cv < cv_threshold → mean; cv ≥ cv_threshold → p85. EWMA: load ewma_mean_tokens, ewma_cv, sample_count, ewma_alpha from rate_calibration_claude.json at startup. Round-sizing heuristic: max_tasks_by_budget = floor(orchestrator_threshold_tokens / pct_cost); pause if effective_rate × remaining_minutes < 3 × pct_cost. First-agent-alone spawn gating has been disabled (see `commands/claude/orchestrator/rate_pacing.md`).
