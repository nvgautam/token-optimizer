...
- Auth enforced on all paths that require it — check for missing auth decorators, unguarded routes, or conditional auth logic that can be bypassed
- No sensitive data written to logs
- Compliance constraints encoded as tests

## Agentic security checks (run on every PR — flag only if violated)
- Prompt injection: external data (API responses, file content, user input) must not be interpolated directly into prompts or agent instructions without sanitization
- Data leakage: PII, secrets, or internal state must not appear in agent output, context files (.ctx.md), or logs — check that sensitive fields are redacted before being passed to the compressor or written to disk
- Context poisoning: content written to .ctx.md files must be generated by the compressor only — flag any code path that writes to context files directly from external input
- Completion signal injection: [TASK_COMPLETE] and [TASK_FAILED] sentinel patterns must only be emitted by the agent, never constructed from external data (e.g. file contents, API responses)
- Scope creep: every file written in this diff must be declared in the task's owns list — flag any write to a path not in owns
- Credential leakage into transcript: flag any code path where env vars, API keys, or stack traces could appear in agent stdout and be captured in the transcript buffer
- Inter-agent trust: task file status fields (done, blocked, failed) must only be updated by the owning worker — flag any code that reads another worker's task file and writes back to it
- Unbounded tool use: flag any subprocess call or outbound HTTP request not implied by the task's declared scope in tasks.json

## Timing and logic checks
- Timing attacks: flag any secret comparison not using a constant-time equality function (e.g. raw == on tokens, passwords, or HMAC digests)
- Auth bypass via logic: flag conditional auth paths where a missing, null, or unexpected value causes the auth check to be skipped rather than rejected (e.g. `if token and validate(token)` where missing token passes)
